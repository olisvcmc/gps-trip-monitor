...
var usingFallback = false; // Flag apakah sedang pakai fallback
    ...
    if (hasBgPlugin) {
        ...
        if (!bgGeoConfigured) {
            setupBackgroundGeolocation(function () {
                ...
                // Fallback timer: jika 10 detik tidak ada update, pakai navigator.geolocation
                startFallbackTimer();
            });
        } else {
            bgGeo.start();
...
function startFallbackTimer() {
    ...
    bgGeoFallbackTimer = setTimeout(function () {
        if (tripPoints.length === 0 && tripState === 'tracking' && !usingFallback) {
            ...
            usingFallback = true;
            // Hentikan plugin yang tidak merespons
...
// Mulai watchPosition foreground sebagai fallback
